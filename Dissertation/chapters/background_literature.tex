...
Further to understanding the foundational concepts of transactions as seen above, we need to understand the underlying concepts of transaction verification. The algorithm used to verify blockchain transactions is known as asymmetric encryption more widely referred to as public-key cryptography \autocite{champagneBookSatoshiCollected2014}. Within the Bitcoin network, a user or node has both a private and public key, their private key is known as a wallet address, and this key is used to prove ownership of existing assets in the network and to sign transactions \autocite{imteajIntroductionBlockchainTechnology2021}. The public key is used by nodes in the network to verify the validity of the transaction by decrypting the privately signed signature with the public key \autocite{antonopoulosMasteringBitcoin2017} \autocite{nakovPracticalCryptographyDevelopers2018}. In permissioned blockchains, organisations may want to extend their existing public key architecture to issue user credentials rather than users managing their keys \autocite{yagaBlockchainTechnologyOverview2018}.  Satoshi Nakamoto when creating Bitcoin decided to use the Elliptic Curve Digital Signature Algorithm (ECDSA) due to its compact file size in an attempt to limit the size of transactions across the network \autocite{johnsonEllipticCurveDigital2001} \autocite{antonopoulosMasteringBitcoin2017}.
